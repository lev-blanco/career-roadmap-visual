...
const impactStyles = {
  ...
  high: { bg: "bg-amber/10", text: "text-amber", border: "border-amber/30", label: "High Impact" },
  medium: { bg: "bg-slate-100", text: "text-slate-500", border: "border-slate-200", label: "Medium Impact" },
};
    ...
    <section id="portfolio" className="py-20 bg-white">
      <div className="container">
        <div className="max-w-xl mb-12">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-3">Portfolio Projects</h2>
          ...
          {portfolioProjects.map((project, i) => {
            ...
            return (
              <motion.div
                ...
                className="group flex flex-col md:flex-row md:items-start gap-4 bg-white border border-slate-200 rounded-xl p-5 hover:border-slate-300 hover:shadow-sm transition-all"
              >
                {/* Rank indicator */}
                <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-slate-50 border border-slate-200 shrink-0">
                  {i === 0 ? (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {project.tags.map((tag) => (
                      <span
                        key={tag}
                        className="font-mono text-[10px] px-2 py-0.5 rounded bg-slate-100 text-slate-500 border border-slate-200"
                      >
                        {tag}
                      </span>
                    ))}
